...
But where could the ciphertext be? As previously mentioned, `4 - Criminal.txt` appears to only consist of 128 bytes of 0x0. Maybe that is what the memdump is for? After all, the ciphertext must be stored _somewhere_ in memory.
